...
y_input = tf.placeholder(tf.float32,'y_input')

#y_output = W * x +b
y_output = tf.add(x=tf.multiply(W,x,'multiply'),y=b,name='y_output')

# loss function
loss = tf.reduce_sum(tf.square(y_output-y_input),'loss')
...
tf.train.write_graph(graph_or_graph_def=session.graph_def,logdir='.',name ='linear_regression_android.pbtxt',as_text=False)
